...
export class FeatureController {
  ...
  async find_cap(): Promise<any> {
    ...
    const result = [];
    let temporary_capabilities = [];

    for (const element in features) {
      if (
        features[element].capability_ids &&
        features[element].capability_ids.length > 0
      ) {
        for (const el in features[element].capability_ids) {
          console.log('ID', features[element].capability_ids[el].valueOf());

          const temporary_capability_object =
            await this.featureService.findCapabilityById({
              id: features[element].capability_ids[el].valueOf(),
            });
