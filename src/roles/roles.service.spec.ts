...
import { ArrayOfObjectRole, MockId, RolePayload, StringMockId, SuccsessCreateRole, IdDependencyAsal, SuccsessGetRoleById, SuccsessUpdateRole, RolePayloadWithRealIdDependency, PayloadOnlyIdDependency, RolePayloadWithAsalIdDependency, SuccsessCreateRoleRealIdDependency } from './mocks/role-payload.mocks';
...
describe('RolesService', () => {
  ...
  it('should update a role', async () => {
    expect(await service.update(MockId,RolePayload)).toEqual(SuccsessUpdateRole(StringMockId));
  });

  it('should get a role', async () => {
    expect(await service.findById(MockId)).toEqual(SuccsessGetRoleById(StringMockId));
  });
  ...
  it('should delete a role', async () => {
    expect(await service.delete(MockId)).toEqual(SuccsessGetRoleById(StringMockId));
  });

  it('should create a role & its dependency', async () => {
    expect(await service.create_dep(RolePayloadWithRealIdDependency)).toEqual(SuccsessCreateRoleRealIdDependency);
  });
  ...
  it('should update a role & its dependency when role sent', async () => {
    expect(await service.update_dep(MockId, RolePayloadWithRealIdDependency )).toEqual(SuccsessUpdateRole(StringMockId));
  });

  it('should update a role & its dependency when role not sent', async () => {
    expect(await service.update_dep(MockId, PayloadOnlyIdDependency)).toEqual(SuccsessUpdateRole(StringMockId) );
  });
