...
import { ArrayOfObjectNavigation, MockId, NavigationPayload, SuccsessCreateNavigation, SuccsessGetNavigationById, SuccsessUpdateNavigation, StringMockId } from './mocks/navigation-payload.mocks';
...
describe('NavigationService', () => {
  ...
  it('should create a navigation', async () => {
    expect(await service.create(NavigationPayload)).toEqual(SuccsessCreateNavigation);
  });

  it('should update a navigation', async () => {
    expect(await service.update(MockId,NavigationPayload)).toEqual(SuccsessUpdateNavigation(StringMockId));
  });

  it('should get a navigation', async () => {
    expect(await service.findById(MockId)).toEqual(SuccsessGetNavigationById(StringMockId));
  });

  it('should get list of navigations', async () => {
    expect(await service.find({ name:'test' })).toEqual(ArrayOfObjectNavigation);
  });

  it('should delete a navigation', async () => {
    expect(await service.delete(MockId)).toEqual(SuccsessGetNavigationById(StringMockId));
  });

  it('should get a feature', async () => {
    expect(await service.findFeatureById(MockId)).toEqual(SuccsessGetFeatureById(StringMockId));
  });
