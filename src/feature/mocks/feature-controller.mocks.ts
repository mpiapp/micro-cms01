import { ArrayOfObjecFeature, SuccsessCreateFeature, SuccsessCreateFeatureWithUndID, SuccsessCreateFeatureWithUndIDWithoutCI } from "./feature-payload.mocks"

export const FeatureControllerMock = {
    // ==================================== controller ====================================
    create: jest.fn().mockImplementation((dto) => { return { id: expect.anything(), ...dto } }),
    findById: jest.fn().mockImplementation((id) => {
        if( ['123','456'].includes(id) ) return { 
            id, ...SuccsessCreateFeatureWithUndID 
        }
    ...
    findByIdAndDelete: jest.fn().mockImplementation((id) => { return { id: id.id, ...SuccsessCreateFeature } })
}